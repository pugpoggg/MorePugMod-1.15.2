package com.pugs.morepugs.entities;
...
public class FlyingPug extends TameableEntity {
   private static final DataParameter<Boolean> BEGGING = EntityDataManager.createKey(FlyingPug.class, DataSerializers.BOOLEAN);
   private static final DataParameter<Integer> COLLAR_COLOR = EntityDataManager.createKey(FlyingPug.class, DataSerializers.VARINT);
  // public static final Predicate<LivingEntity> TARGET_ENTITIES = (p_213440_0_) -> {
   ...
   public FlyingPug(EntityType<? extends FlyingPug> type, World worldIn) {
      super(type, worldIn);
      this.setTamed(false);
   }
   ...
   public FlyingPug createChild(AgeableEntity ageable) {
	   FlyingPug flyingpug = EntityInit.PUG.get().create(this.world);
      UUID uuid = this.getOwnerId();
      if (uuid != null) {
         flyingpug.setOwnerId(uuid);
         flyingpug.setTamed(true);
      }
   ...
   public boolean canMateWith(AnimalEntity otherAnimal) {
      if (otherAnimal == this) {
         ...
         return false;
      } else if (!(otherAnimal instanceof FlyingPug)) {
         return false;
      } else {
    	  FlyingPug flyingpug = (FlyingPug)otherAnimal;
         if (!flyingpug.isTamed()) {
            return false;
         } else if (flyingpug.isSitting()) {
            return false;
         } else {
            return this.isInLove() && flyingpug.isInLove();
         }
      }
   }
   ...
   public boolean shouldAttackEntity(LivingEntity target, LivingEntity owner) {
      if (!(target instanceof CreeperEntity) && !(target instanceof GhastEntity)) {
         if (target instanceof FlyingPug) {
            FlyingPug flyingpug = (FlyingPug)target;
            return !flyingpug.isTamed() || flyingpug.getOwner() != owner;
   ...
   class AvoidEntityGoal<T extends LivingEntity> extends net.minecraft.entity.ai.goal.AvoidEntityGoal<T> {
      private final FlyingPug pug;

      public AvoidEntityGoal(FlyingPug pugIn, Class<T> entityClassToAvoidIn, float avoidDistanceIn, double farSpeedIn, double nearSpeedIn) {
         super(pugIn, entityClassToAvoidIn, avoidDistanceIn, farSpeedIn, nearSpeedIn);
         this.pug = pugIn;
      }
      ...
      public void startExecuting() {
         FlyingPug.this.setAttackTarget((LivingEntity)null);
